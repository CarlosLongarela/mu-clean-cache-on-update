 ...
 * Description:     Purge Nginx cache and CloudFlare cache if it's configured.
 * Version:         0.1.0
 * Author:          Carlos Longarela
...
/**
 * Nginx Helper Enhancement for cache purger.
 *
 * @return void
 */
function cl_nginx_cache_purge() {
	wp_cache_flush();
...
add_action( 'upgrader_process_complete', 'cl_nginx_cache_purge', 10, 0 ); // After plugins have been updated.
...
if ( defined( 'ELEMENTOR_VERSION' ) ) {
	...
	add_action( 'update_option__elementor_global_css', 'cl_nginx_cache_purge', 10, 3 );
	add_action( 'delete_option__elementor_global_css', 'cl_nginx_cache_purge', 10, 3 );
}

// AutoOptimizer.
if ( defined( 'AUTOPTIMIZE_PLUGIN_DIR' ) ) {
	add_action( 'autoptimize_action_cachepurged','cl_nginx_cache_purge', 10, 3 );
}
